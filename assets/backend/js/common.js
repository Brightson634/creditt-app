...
$(document).ready(function () {
    ...
    $(document).on(
        ...
        function () {
            var input = $(this).closest('.input-number').find('input.input_number');
            var increase_by_input = $(this).closest('.input-number').find('input.increase_by');
            var decrease_by_input = $(this).closest('.input-number').find('input.decrease_by');
            // console.log(increase_by_input)
            // console.log(input + ' ' + increase_by_input + ' ' + decrease_by_input)

            var qty = __read_number(input);
            // console.log(__read_number(decrease_by_input))
            // console.log(__read_number(increase_by_input))
            // console.log(qty)
            var step = 1;
            if (input.data('step')) {
            ...
            if ($(this).hasClass('quantity-up')) {
                ...
                input.change();
                __write_number(decrease_by_input, 0);
                decrease_by_input.change()
                __write_number(increase_by_input, parseInt(increase_by_input.val()) + 1);
                increase_by_input.change()
                console.log(__read_number(increase_by_input))
                $(this).closest('.input-number').find('.kot_hide').remove();
                ...
                __write_number(input, qty - step);
                input.change();
                __write_number(increase_by_input, 0);
                increase_by_input.change()
                __write_number(decrease_by_input, parseInt(decrease_by_input.val()) + 1);
                decrease_by_input.change()
                $(this).closest('.input-number').find('.kot_hide').remove();
