    ...
    public function AccountsDropdown()
    {
        if (request()->ajax()) {
            ...
            foreach ($accounts as $account) {
                ...
                $accounts_array[] = [
                    'id' => $account->id,
                    'text' =>'<span style="font-size:12px">'.               $account->name.   '</span>'.' - <small
                        class="text-muted">'.$account->account_primary_type.' - '. $translatedText.'</small>',
                    'html' =>'<span style="font-size:12px">'. $account->name.'</span>'.' - <small class="text-muted"
