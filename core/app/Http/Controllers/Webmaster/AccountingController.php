...
class AccountingController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        $colors = [
            '#E75E82', '#37A2EC', '#FACD56', '#5CA85C', '#605CA8', '#2f7ed8', '#0d233a', '#8bbc21', '#910000', '#1aadce', '#492970', '#f28f43', '#77a1e5', '#c42525', '#a6c96a',
        ];
    ...
    public function AccountsDropdown()
    {
        if (request()->ajax()) {
            ...
            foreach ($accounts as $account) {
                ...
                $accounts_array[] = [
                    'id' => $account->id,
                    'text' =>$account->name .   ':' . ' -' . $account->account_primary_type . ' - '. $translatedText,
                    'html' => '<span class="dropdownAccount"style="font-size:12px" data-currency='.$account->account_currency.'>' . $account->name . '</span>' . '  <small class="text-muted"
                            style="font-size:10px">' . $account->account_primary_type . ' - ' .
