...
class BusinessUtil extends Util
{
    ...
    public function checkUserSubscriptionHasAccessToModule($module){
        ...
        //No Active subscription for the Module on Package
        if(is_null($package)) {  return false;  }

        //if package then check if a module is enabled

        $custom_permissions = $package->package_details;

        // dd((int)$custom_permissions[$module] , in_array($module , array_keys($custom_permissions)));

        if(in_array($module , array_keys($custom_permissions))){

            //module exisits and is enabled on the package

            // dd($custom_permissions);

            return intval($custom_permissions[$module]);


        }

        // no modules found an no package
        return false;
