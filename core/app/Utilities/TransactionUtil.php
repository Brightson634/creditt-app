...
class TransactionUtil extends Util
{
    ...
    public $previousSellLineBeforeUpdate = [];
    /**
     ...
     */
    public function createOrUpdateSellLines($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true , $input_data = [])
    {
        $lines_formatted = [];
        $modifiers_array = [];
        $edit_ids = [0];
        ...
        foreach ($products as $product) {
            ...
            $multiple_staff = isset($product['res_service_staff_id']) && is_array($product['res_service_staff_id'])? true : false;
            $multiplier = 1;
            ...
            if (! empty($product['transaction_sell_lines_id'])) {
                ...
                $this->previousSellLineBeforeUpdate[] = $product;
            } else {
                ...
                if($multiple_staff){
                    ...
                    foreach ($product['res_service_staff_id'] as $staff_id) {
                        ...
                        $line = [
                            ...
                            'res_service_staff_id' => $staff_id?? null,
                            'res_service_staff_commission' => $commission_per_member ?? null,
                    ...
                    //contine as was without multiple staff
                    ...
                    $line = [
                        ...
                        'res_service_staff_commission' => $product['res_service_staff_commission'] ?? null,
                        'res_line_order_status' => ! empty($product['res_service_staff_id']) ? 'received' : null,
                        'so_line_id' => ! empty($product['so_line_id']) ? $product['so_line_id'] : null,
                        'secondary_unit_quantity' => ! empty($product['secondary_unit_quantity']) ? $this->num_uf($product['secondary_unit_quantity']) : 0,
                    ];
     ...
     * @return bool
     */
    public function editSellLine($product, $location_id, $status_before, $multiplier = 1, $uf_data = true , $transaction = null)
    {
        $multiple_staff = is_array($product['res_service_staff_id'])? true : false;

        //check if product has multiple sell lines this indicates multiple service staff
        ...
        $sell_line->fill(['product_id' => $product['product_id'],
            ...
            'sell_line_note' => ! empty($product['sell_line_note']) ? $product['sell_line_note'] : '',
            'sub_unit_id' => ! empty($product['sub_unit_id']) ? $product['sub_unit_id'] : null,
            'res_service_staff_id' => $product['res_service_staff_id'][$key],
            'res_service_staff_commission' => $commission_per_member,
            'secondary_unit_quantity' => ! empty($product['secondary_unit_quantity']) ? $this->num_uf($product['secondary_unit_quantity']) : 0,
        ]);
        $sell_line->save();
     ...
     * @return array
     */
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type , $hide =[] ,$inc_data=[] ,$dec_data=[] )
    {
        $il = $invoice_layout;
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            $output['tax_summary_label'] = $il->common_settings['tax_summary_label'] ?? '';
                ...
                //everything in the array hide is filtered out.
                if(count($hide)){
                    $lines = $lines->filter(function($line)use($hide){
                        return !in_array($line->product_id , $hide);
                    });
                }
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            $line_array = [
                ...
                'kitchen_preparation_required' => $product->kitchen_preparation_required,
                'name' => $product->name,
    ...
    public function getTotalPaymentWithCommission($business_id, $start_date = null, $end_date = null, $location_id = null, $commission_agent = null)
    {
        ...
        if (! empty($commission_agent)) {
            ...
            $business_details = (new BusinessUtil())->getDetails($business_id);
            $pos_settings = empty($business_details->pos_settings) ? (new BusinessUtil())->defaultPosSettings() : json_decode($business_details->pos_settings, true);

            if (($pos_settings['cmmsn_calculation_type'] ?? '') !== 'per_product'){
                $query->where('t.commission_agent', $commission_agent);
            } else {
                $query->leftJoin('transaction_sell_lines as tsl', 'transaction_payments.transaction_id', '=', 'tsl.transaction_id')
                    ->where('tsl.res_service_staff_id',  $commission_agent);
            }
        }
    ...
    public function getTotalCommission($business_id, $start_date = null, $end_date = null, $location_id = null, $commission_agent = null, $pos_settings = null)
    {
        $query = TransactionPayment::join('transactions as t', 'transaction_payments.transaction_id', '=', 't.id')
            ->where('t.business_id', $business_id)
            ->where('t.type', 'sell')
            ->where('t.status', 'final')
            ->leftJoin('transaction_sell_lines as tsl', 'transaction_payments.transaction_id', '=', 'tsl.transaction_id')
            ->select(DB::raw('SUM(IF( is_return = 0, tsl.res_service_staff_commission, tsl.res_service_staff_commission*-1)) as total_paid'));
        ...
        if (! empty($commission_agent)) {
            ...
            if (($pos_settings['cmmsn_calculation_type'] ?? '') !== 'per_product'){
                ...
                $query->where('tsl.res_service_staff_id',  $commission_agent);
            }
        }

        $payment_details = $query->first();

        $output['total_commission'] = $payment_details->total_paid;
