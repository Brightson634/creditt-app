...
class Variation extends Model
{
    ...
    public function commission($unit_price_inc_tax, $percent = false) : int|string
    {
        $commission = $this->product->commission ?? 0;
        $price = $this->quantity_ordered * $unit_price_inc_tax;

        if (strtolower($this->product->commission_type) == 'percentage') {
            return $percent ? "{$commission}%" : $price * ($commission / 100);
        } elseif (strtolower($this->product->commission_type) == 'fixed') {
            return $commission;
        }
