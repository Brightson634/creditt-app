...
class BusinessLocation extends Model
{
    ...
    public static function forDropdownWithStores($business_id, $show_all = false, $receipt_printer_type_attribute = false, $append_id = true, $check_permission = true)
    {
        $query = BusinessLocation::query()
        // ->where('is_store',0)
        ->where('business_id', $business_id)->Active();
    ...
    public function location_stores(){
        return $this->hasMany(LocationStore::class,'location_id');
    }
    ...
    public function stores(){
        ...
        $this->hasMany(BusinessLocation::class, 'parent_id');
    }


    public function main_store(){

        return $this->belongsTo(BusinessLocation::class, 'parent_id')->with('stores');
    }
}
