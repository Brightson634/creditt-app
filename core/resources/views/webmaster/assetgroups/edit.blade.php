                ...
                <div class="card-body">
                    <div class="clearfix mb-3">
                        <div class="float-left">
                            <h3 class="card-title">Edit Asset Group Information</h3>
                        </div>
                    ...
                    <form action="#" method="POST" id="assetgroup_form">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" value="{{$asset_group->name}}" name="name" id="name" class="form-control">
                            <span class="invalid-feedback"></span>
                        </div>
                        <input type='hidden' value="{{$asset_group->id}}" name='id'>

                        <div class="form-group">
                            <label for="description">Description</label>
                            <textarea name="description" class="form-control" id="description" rows="3">{{$asset_group->description}}</textarea>
                            <span class="invalid-feedback"></span>
                        </div>
                        <div class="form-group row mt-4">
                            <div class="col-sm-9">
                                <button type="submit" class="btn btn-theme btn-primary" id="btn_assetgroup">Update Asset group</button>
                            </div>
                        </div>
                    </form>
                </div>
...
@section('scripts')
    <script type="text/javascript">
        $("#assetgroup_form").submit(function(e) {
            ...
            $.ajax({
                url: '{{ route('webmaster.assetgroup.update') }}',
                method: 'post',
                data: $(this).serialize(),
                dataType: 'json',
                success: function(response) {
                    if (response.status == 400) {
                        ...
                        $("#btn_assetgroup").html('Update Asset Group');
