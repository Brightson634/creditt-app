...
@section('content')
            ...
            <div class="col-md-12">
                @component('webmaster.components.widget', ['class' => 'box-solid'])
                    ...
                    @slot('tool')
                        <div class="box-tools">
                            <button type="button"
                                class="btn btn-primary tw-to-blue-500 tw-font-bold tw-text-white tw-border-none tw-rounded-full pull-right btn-modal"
    ...
    <div id="updateTransfer" class="modal">
        <div class="modal-dialog" role="document" id='transferContent'>
        </div><!-- modal-dialog -->
    </div><!-- modal -->
@stop
        ...
        $(document).ready(function() {
            ...
            transfer_table = $('#transfer_table').DataTable({
                ...
                columns: [{
                    ...
                    {
                        data: 'note',
                        name: 'accounting_acc_trans_mappings.note'
                    }
                ]
            });
            ...
            $('#transfer_date_range_filter').daterangepicker(
                // dateRangeSettings,
                function(start, end) {
                    $('#transfer_date_range_filter').val(start.format(moment_date_format) + ' ~ ' + end.format(
                        moment_date_format));
                    transfer_table.ajax.reload();
                }
            );
            ...
            //updating transfer
            $(document).on('click', '.transferUpdate', function() {
                const url = $(this).data('href')
                $.ajax({
                    type: "get",
                    url: url,
                    success: function(response) {
                        console.log(response);
                        $('#transferContent').html(response.html);
                        $('#updateTransfer').modal('show');
