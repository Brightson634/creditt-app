        ...
        <div class="tab-content">
            ...
            <div class="tab-pane" id="yearly_tab">
                <div class="text-right mb-12">
                    ...
                    <a class="btn btn-sm btn-default"
                        href="{{ route('webmaster.budget.index') }}?financial_year={{ $fy_year }}&format=csv&view_type=yearly">
                        <i class="fas fa-file-csv">Export to csv
                    </a>
                    <a class="btn btn-sm btn-default"
                        href="{{ route('webmaster.budget.index') }}?financial_year={{ $fy_year }}&format=excel&view_type=yearly">
                        <i class="fas fa-file-excel"></i>Export to excel
                        ...
                        <tbody class="collapsed">
                            ...
                            @foreach($accounts->where('account_primary_type', $account_type)->sortBy('name') as $account)
                                ...
                                <tr class="collapse-tr">
                                    <th>{{ $account->name }}</th>
                                    <td>
                                        @if(!is_null($account_budget) && !is_null($account_budget->amount))
                                            {{ number_format($account_budget->amount, 2) }}
                                        @else
                                            0
                                        @endif
                                    </td>
                            ...
                            <tr class="collapse-tr bg-gray">
                                <th>@lang('sale.total')</th>
                                <td>{{ number_format($budget->whereIn('accounting_account_id', $account_ids)->sum('amount'), 2) }}</td>
                            </tr>
                        </tbody>
                        @endforeach
                        <tfoot class="bg-green">
                            <tr class="table-footer">
                                <th>Grand Total</th>
                                <td>{{ number_format($budget->sum('amount'), 2) }}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
